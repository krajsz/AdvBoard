...
void AbstractDashboard::setAllowDraggin(bool allow)
{
	m_allowDragging = allow;
}

bool AbstractDashboard::allowDragging() const
{
	return m_allowDragging;
}
...
void AbstractDashboard::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
	if (m_allowDragging)
	{
		...
		for (const AdvSensorItem* const item : sensorItems())
		{
			if (item->boundingRect().contains(item->mapFromParent(event->pos())))
			{
				qDebug() << "sensor move: " << item->sensor()->type();
...
void AbstractDashboard::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
	qDebug() << "released dashboard";
